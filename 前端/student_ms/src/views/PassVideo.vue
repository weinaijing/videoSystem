<template>
    <div>
        ...
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item>过审视频</el-breadcrumb-item>
        </el-breadcrumb>
        <!-- 卡片布局 -->
        <el-card class="box-card">
            ...
            <el-row :gutter="20">
                <el-col :span="7">
                    <el-input v-model="queryObj.query" placeholder="请输入视频名称" class="input-with-select">
                        <template #append>
                            <el-button icon="Search" @click="getPassVideoList" />
                        </template>
                    </el-input>
                </el-col>
                <el-col :span="2">
            ...
            <!-- 表格数据区域 -->
            <el-table :data="passVideoList" border stripe v-loading="listLoading"
                ...
                <el-table-column label="操作" width="180px">
                    <template #default="scope">
                        ...
                        <el-button type="danger" icon="delete" @click="deletePassVideo(scope.row.id)"></el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog v-model="AuditDialogVisible" title="视频二次审核" width="50%" :before-close="handleClose">
            <span>
...
export default {
    ...
    created() {
        this.getPassVideoList();
        
    },
    methods: {
        // 获取过审视频列表的方法
        async getPassVideoList() {
            const result = await this.$http.get('/getPassVideos', { params: this.queryObj });
            console.log(result);
            if (result.status == 200) {
                this.passVideoList = result.data.data;
        ...
        deletePassVideo(id) {
            ...
            if (state == '1') {
                ...
                this.$confirm('此操作将永久删除视频, 是否继续?', '提示', {
                    ...
                    const result = await this.$http.delete('/deletePassVideo/' + id);
                    console.log(result);
                    if (result.status !== 200) {
        ...
        async delAll() {
            ...
            if (state == '1') {
                ...
                this.$confirm('此操作将永久删除用户, 是否继续?', '提示', {
                    ...
                    const result = await this.$http.delete('/deletePassVideos/' + arr);
                    console.log(result);
                    if (result.status !== 200) {
                        return this.$message.error('删除失败！');
                    }
