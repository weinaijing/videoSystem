<template>
    <div>
            ...
            <el-row :gutter="20">
                <el-col :span="7">
                    <el-input v-model="queryObj.query" placeholder="请输入视频名称" class="input-with-select">
                        <template #append>
                            <el-button icon="Search" @click="getReportVideo" />
                        </template>
                    </el-input>
                </el-col>
                <el-col :span="2">
        ...
        <el-dialog v-model="AuditDialogVisible" title="举报视频审核" width="50%" :before-close="handleClose">
            <span>
            ...
            <span class="dialog-footer">
                ...
                <el-button type="primary" @click="PassReportVideo()">通过审核</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        // 获取被举报视频列表的方法
        async getReportVideo() {
            const result = await this.$http.get('/getReportVideo', { params: this.queryObj });
            console.log(result);
        ...
        deleteVideo(id) {
            ...
            if (state == '1') {
                ...
                this.$confirm('此操作将永久删除视频, 是否继续?', '提示', {
                    ...
                    const result = await this.$http.delete('/deleteReportVideo/' + id);
                    console.log(result);
                    if (result.status !== 200) {
        ...
        async delAll() {
            ...
            if (state == '1') {
                ...
                this.$confirm('此操作将永久删除用户, 是否继续?', '提示', {
                    ...
                    const result = await this.$http.delete('/PassReportVideo/' + arr);
                    console.log(result);
                    if (result.status !== 200) {
                        return this.$message.error('删除失败！');
                    }
        ...
        PassReportVideo() {
            ...
            if (state == '1') {
                ...
                this.$confirm('此操作将永久删除用户, 是否继续?', '提示', {
                    ...
                    const result = await this.$http.delete('/PassReportVideo/' + this.AuditvideoList.id);
                    console.log(result);
                    if (result.status !== 200) {
                        return this.$message.error('删除失败！');
                    }
